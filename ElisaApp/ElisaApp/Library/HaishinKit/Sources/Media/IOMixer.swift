...
public class IOMixer {
	...
	var syncDiff = CMTime.zero
	var audioBufferEmitter = PublishSubject<CMSampleBuffer>()
	var frameSkipBeforeTrack = 60
	//-END SPRII
	...
	func useSampleBuffer(sampleBuffer: CMSampleBuffer, mediaType: AVMediaType) -> Bool {
		...
		case .video:
			if mediaType == .audio {
				lastAudioTimeStamp = sampleBuffer.presentationTimeStamp
			...
			frameSkipBeforeTrack -= 1
			if frameSkipBeforeTrack == 0 {
				if lastAudioTimeStamp != .zero {
					syncDiff = lastAudioTimeStamp - sampleBuffer.presentationTimeStamp
				} else {
					frameSkipBeforeTrack = 60
				}
				print("\n-->> DELAY: \(syncDiff.seconds)\n")
			}
