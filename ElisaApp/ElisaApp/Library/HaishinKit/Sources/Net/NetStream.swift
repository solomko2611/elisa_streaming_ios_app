...
open class NetStream: NSObject {
	...
	/// //-START SPRII
	open func attachCamera(_ device: AVCaptureDevice?, onError: ((_ error: Error) -> Void)? = nil, onCompletion: ((AVCaptureVideoPreviewLayer?) -> Void)? = nil) {
		lockQueue.async {
			do {
				let previewLayer = try self.mixer.videoIO.attachCamera(device)
				onCompletion?(previewLayer)
			} catch {
				onError?(error)
			}
		}
